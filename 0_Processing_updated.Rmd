...
# 1) Triples: System → IPCC → GGA
triples <- adaptdata %>%
  mutate(across(c(SystemType, SectorType, SectorType_GGA), ~na_if(., ""))) %>%
  filter(!is.na(SystemType), !is.na(SectorType), !is.na(SectorType_GGA)) %>%
  separate_rows(SystemType,     sep = ";") %>%
  separate_rows(SectorType,     sep = ";") %>%
  separate_rows(SectorType_GGA, sep = ";") %>%
  mutate(across(c(SystemType, SectorType, SectorType_GGA), ~str_trim(.))) %>%
  count(SystemType, SectorType, SectorType_GGA, name = "value") %>%
  arrange(desc(value))

# 2) Edge lists for Sankimatic (From, To, Value)
edges_sys_ipcc <- triples %>%
  count(SystemType, SectorType, wt = value, name = "value") %>%
  rename(from = SystemType, to = SectorType)

edges_ipcc_gga <- triples %>%
  count(SectorType, SectorType_GGA, wt = value, name = "value") %>%
  rename(from = SectorType, to = SectorType_GGA)

write.table(edges_ipcc_gga, "edges_ipcc_gga.txt",
            sep = " ", row.names = FALSE, col.names = TRUE, quote = FALSE,
            fileEncoding = "UTF-8")

# Build Sankimatic-formatted lines: "From [value] To"
sankimatic_lines <- edges_sys_ipcc %>%
  dplyr::mutate(
    from = trimws(as.character(from)),
    to   = trimws(as.character(to)),
    line = paste0(from, " [", value, "] ", to)
  ) %>%
  dplyr::pull(line)

# Preview in console
cat(paste(sankimatic_lines, collapse = "\n"))

# Write to a plain text file
writeLines(sankimatic_lines, "edges_sys_ipcc_sankimatic.txt", useBytes = TRUE)
