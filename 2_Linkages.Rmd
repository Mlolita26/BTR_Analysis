...
linkage_evidence <- rows_aug %>%
  ...
  group_modify(~ tibble(
    ...
    Evidence = c(
      first_snippet(.x %>% filter(Element == "System at risk"),        RowHasHazardTerms),
      ...
      first_snippet(.x %>% filter(Element == "Result"),                RowHasHazardTerms),   # NOT "Indicator"
      ...
      first_snippet(.x %>% filter(Element == "Result"),                RowHasSystemTerms)    # NOT "Indicator"
    )
  )) %>% ungroup()

# Pivot evidence wide & attach to summary
